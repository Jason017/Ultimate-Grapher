...
public class MatrixStorage {
    ...
    public MatrixStorage(Coordinates[] coordParam, int thresholdParam, double maxErrorParam){
        mValue = getNumOfCoords(); //sets up the row value for the matrix
        nValue = thresholdParam + 1; //sets up the column value for the matrix
     ...
     * checks to make sure that coordinates inputed by the user can be used for later calculations
     */
    ...
    /**
     * (Add one to threshold)
     */
    public void addToThreshold(){
        threshold++;
    }
